...
select * from sharks_data;

ALTER TABLE sharks_data
CHANGE COLUMN ï»¿Episode emp_id int NULL;


-- total episodes

select count(distinct emp_id) from sharks_data;

-- pitches 

select count(distinct brand) from sharks_data;

 -- from total pitches how many get funding converted
 
 select sum(a.converted_not_converted ) as get_funding,count(*) as total_pitches from
 (select Amount_invested_lakhs , case when Amount_invested_lakhs>0 then 1 else 0 end as converted_not_converted from sharks_data) a;
